...
@foreach($categoryNews as $category)
  ...
  <div class="row">
    <div class="col-lg-9" style="background-color:yellow;" >
    <div class="conatiner">{{ $category->category_name }}</div>
    <div class="col-lg-9">
    <table class="table border_bottom">
     @foreach($category->news->load('user') as $news)
      <tr>
          <td> <img src="{{$news->getImage()}}" class="img-responsive"></td>
          <td>{{ $news->title }}</td>
          <td><p>{{ $news->body }}</p><a class="btn btn-default" >Created By {{ $news->user->firstname }}</p></td>
      </tr>
      @endforeach
